...
local helljumperFontOverride = {
    ...
    small_font_family = "NexaDemo-Bold",
    ...
    smaller_font_family = "NexaDemo-Bold",
    ...
    ticker_font_family = "NexaDemo-Bold",
...
function chimera.setupFonts(revert)
    ...
    if chimeraIni then
        ...
        if configuration then
            configuration.font_override = helljumperFontOverride
            if revert then
...
function chimera.fontOverride()
    ...
    if create_font_override then
        if constants.fonts.text then
            safeFontOverride(constants.fonts.text.id, "NexaDemo-Bold", 14, 400, 2, 2, 1, 1)
        end
        if constants.fonts.title then
            safeFontOverride(constants.fonts.title.id, "NexaDemo-Bold", 18, 400, 2, 2, 0, 0)
        end
        if constants.fonts.subtitle then
            safeFontOverride(constants.fonts.subtitle.id, "NexaDemo-Bold", 10, 400, 2, 2, 0, 0)
        end
        if constants.fonts.button then
            safeFontOverride(constants.fonts.button.id, "NexaDemo-Bold", 13, 400, 2, 2, 1, 1)
        end
        if constants.fonts.shadow then
            safeFontOverride(constants.fonts.shadow.id, "NexaDemo-Bold", 10, 400, 0, 0, 0, 0)
        end
        return true
    end
